...
import { TooltipProvider, TooltipSimple, Tooltip, TooltipTrigger, TooltipContent } from "@opc/ui/tooltip-modern";
import { FilePicker } from "./FilePicker";
import { SlashCommandPicker } from "./SlashCommandPicker";
import { ImagePreview } from "./ImagePreview";
import { type FileEntry, type SlashCommand } from "@/lib/api";

// Lazily get the Tauri webview window at call time (Tauri internals may not be
// ready at module-evaluation time, so we cannot do a top-level require check).
async function getTauriWebviewWindow(): Promise<{ onDragDropEvent: ((...args: any[]) => any) | undefined; listen: (...args: any[]) => Promise<() => void> } | null> {
  try {
    if (typeof window !== 'undefined' && ((window as any).__TAURI_INTERNALS__ || (window as any).__TAURI__)) {
      const { getCurrentWebviewWindow } = await import("@tauri-apps/api/webviewWindow");
      return getCurrentWebviewWindow() as any;
    }
  } catch {
    // not in Tauri
  }
  ...
  useEffect(() => {
    ...
    const setupListener = async () => {
      try {
        ...
        const webview = await getTauriWebviewWindow();
