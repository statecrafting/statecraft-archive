...
- A package landing here brings its own spec, its own LICENSE file, an entry in
`statecrafting.licenseTiers` in the root `package.json`, and an entry in both
`standalone_rust_workspaces` and `standalone_npm_packages`.
...
A **record** in this corpus uses that last row deliberately. Spec 001 is the
migration map, not a work order, and carries no `implementation` key at all:
statecraft's thesis spec set `implementation: pending` on a record and then
needed a paragraph in its `AGENTS.md` warning agents not to pick it up. A
record has no implementation state.
...
declare keys, write down their semantics here or in the constitution, next to
the rest of what governs the corpus.

This corpus declares none. Every key used in `specs/*/spec.md` is grammar.
