...
/// Returns the path regardless of whether it exists — callers must check existence.
...
/// Built `dist/node-sidecar.js` from `product/packages/provider-registry`
/// (spec 042 Phase 6). The walk-up from CARGO_MANIFEST_DIR=
/// `product/apps/desktop/src-tauri` lands at `product/`, then we join
/// `packages/provider-registry/dist/node-sidecar.js`.
fn bridge_sidecar_js_path() -> Result<PathBuf, String> {
    ...
    if !p.exists() {
        return Err(format!(
            "Bridge sidecar not found at {}. Build with: pnpm exec tsc -p product/packages/provider-registry/tsconfig.json",
            p.display()
        ));
    }
    Ok(p)
...
fn extract_first_user_message(jsonl_path: &PathBuf) -> (Option<String>, Option<String>) {
    ...
    for line in reader.lines() {
        if let Ok(line) = line
            && let Ok(entry) = serde_json::from_str::<JsonlEntry>(&line)
            ...
            if let Ok(session_entries) = fs::read_dir(&path) {
                for session_entry in session_entries.flatten() {
                    ...
                    if session_path.is_file()
                        ...
                        && let Some(session_id) = session_path.file_stem().and_then(|s| s.to_str())
                    {
...
pub async fn delete_session(project_id: String, session_id: String) -> Result<(), String> {
    ...
    fs::remove_file(&session_file).map_err(|e| format!("Failed to delete session file: {}", e))?;
...
pub async fn delete_checkpoint(
    ...
    log::info!(
        "Deleting checkpoint {} for session {}",
        checkpoint_id,
        session_id
    );
...
pub async fn get_claude_settings() -> Result<ClaudeSettings, String> {
    ...
    if !claude_dir.exists() || !settings_path.exists() {
...
pub async fn save_system_prompt(content: String) -> Result<String, String> {
...

    {
        let parent = claude_md_path
            .parent()
            .ok_or("CLAUDE.md path has no parent")?;
        let mut tmp = tempfile::NamedTempFile::new_in(parent)
            .map_err(|e| format!("Failed to create temp file: {}", e))?;
        tmp.write_all(content.as_bytes())
            .map_err(|e| format!("Failed to write temp file: {}", e))?;
        tmp.persist(&claude_md_path)
            .map_err(|e| format!("Failed to persist CLAUDE.md: {}", e))?;
    }
...
pub async fn execute_claude_code(
    ...
    if let Some(reason) = &bypass_reason {
        eprintln!(
            "[governance] execute_claude_code falling back to bypass: {}",
            reason
        );
    }
    let mode = match &plan {
...
pub async fn continue_claude_code(
    ...
    if let Some(reason) = &bypass_reason {
        eprintln!(
            "[governance] continue_claude_code falling back to bypass: {}",
            reason
        );
    }
    let mode = match &plan {
...
pub async fn resume_claude_code(
    ...
    if let Some(reason) = &bypass_reason {
        eprintln!(
            "[governance] resume_claude_code falling back to bypass: {}",
            reason
        );
    }
    let mode = match &plan {
...
/// Run Claude Code / provider-registry via the Node sidecar (`product/packages/provider-registry/dist/node-sidecar.js`) — specs 045 + 042 Phase 6.
#[tauri::command]
pub async fn execute_claude_bridge(
    ...
    if let Some(reason) = &bypass_reason {
        eprintln!(
            "[governance] execute_claude_bridge falling back to bypass: {}",
            reason
        );
    }
    let mode = match &plan {
    ...
    let stdout_task = tokio::spawn(async move {
        ...
        while let Ok(Some(line)) = lines.next_line().await {
            ...
            if let Ok(msg) = serde_json::from_str::<serde_json::Value>(&line)
                && msg["type"] == "system"
                && msg["subtype"] == "init"
                && let Some(claude_session_id) = msg["session_id"].as_str()
            {
    ...
    let stdout_task = tokio::spawn(async move {
        ...
        while let Ok(Some(line)) = lines.next_line().await {
            ...
            if let Ok(v) = serde_json::from_str::<serde_json::Value>(&line)
                && v.get("done").and_then(|x| x.as_bool()) == Some(true)
            {
                break;
            }

            if let Ok(msg) = serde_json::from_str::<serde_json::Value>(&line)
                && msg.get("type") == Some(&serde_json::json!("system"))
                && msg.get("subtype") == Some(&serde_json::json!("init"))
                && let Some(claude_session_id) = msg.get("session_id").and_then(|s| s.as_str())
            {
                let mut session_id_guard = session_id_holder_clone.lock().unwrap();
                if session_id_guard.is_none() {
                    *session_id_guard = Some(claude_session_id.to_string());
                    ...
                    match registry_clone.register_claude_session(
                        ...
                        Ok(run_id) => {
                            log::info!("Registered Claude bridge session with run_id: {}", run_id);
                            let mut run_id_guard = run_id_holder_clone.lock().unwrap();
                            *run_id_guard = Some(run_id);
                        }
                        Err(e) => {
...
pub async fn restore_checkpoint(
    ...
    {
        let parent = session_path.parent().ok_or("Session path has no parent")?;
        let mut tmp = tempfile::NamedTempFile::new_in(parent)
            .map_err(|e| format!("Failed to create temp file: {}", e))?;
        tmp.write_all(messages.as_bytes())
            .map_err(|e| format!("Failed to write temp file: {}", e))?;
