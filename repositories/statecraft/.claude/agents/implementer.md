...
| Your code | `backend/ (Encore.ts services), frontend/, frontend-admin/, infra/, scripts/` | `npm run typecheck && npm test` |
...
After each change:
- **Code**: run your project's build check (e.g. `npm run typecheck && npm test`)
