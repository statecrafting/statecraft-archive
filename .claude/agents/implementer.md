...
- For focused changes: add a page, fix a broken link, update a config, wire a new route
...
| Site code | `app/`, `public/`, `scripts/`, `react-router.config.ts`, `vite.config.ts` | `npm run typecheck && npm run build` |
...
After each change:
- **Code**: run your project's build check (e.g. `npm run typecheck && npm run build`)
