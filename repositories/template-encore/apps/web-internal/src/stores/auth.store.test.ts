 ...
 * fetchUser, login, logout, checkStatus, loggingOut/clearLoggingOut.
 *
...
describe('logout', () => {
  ...
  it('loggingOut starts false', () => {
    const store = useAuthStore()
    expect(store.loggingOut).toBe(false)
  })

  it('loggingOut is true after logout completes (before clearLoggingOut is called)', async () => {
    vi.mocked(axios.get).mockResolvedValue({ data: { token: 'csrf-1' } })
    vi.mocked(axios.post).mockResolvedValue({ data: { success: true } })
    ...
    // Router's afterEach normally clears this; here it should still be set
    expect(store.loggingOut).toBe(true)
  })

  it('clearLoggingOut resets loggingOut to false', async () => {
    vi.mocked(axios.get).mockResolvedValue({ data: { token: 'csrf-1' } })
    vi.mocked(axios.post).mockResolvedValue({ data: { success: true } })
    ...
    store.clearLoggingOut()
    expect(store.loggingOut).toBe(false)
  })

  it('loggingOut is true even when logout throws (router still needs to clear it)', async () => {
    vi.mocked(axios.get).mockResolvedValue({ data: { token: 'csrf-1' } })
    vi.mocked(axios.post).mockRejectedValue({
      response: { data: { code: 'internal', message: 'Logout failed' } },
    })
