# AGENTS.md: statecrafting/.github
...
Governance is provided by `spec-spine`, installed on your `PATH`. This
repository pins a floor of **0.18.0** (`spec-spine.toml [meta]
required_version`), so the CLI refuses an older binary on every run rather than
letting one judge a corpus it does not understand. All governed reads of
compiled artifacts go through the CLI. Bootstrap spec:
`specs/000-bootstrap/spec.md`.

**What this repository is.** The statecrafting organisation's `.github` repo:
the org profile that GitHub renders at github.com/statecrafting, plus the
community health files the org inherits from here. There is no application
code, no Cargo workspace and no npm package. The language halves of the gate
are clean no-ops, guarded on a manifest probe; the governance half is the whole
gate.
...
1. **Parallel reads.** Dispatch the following simultaneously (nothing here
   ...
   - `ls profile/ profile/artifacts/`: the rendered surface
   - `ls specs/`: the corpus inventory on disk
   - `git log --oneline -10`: recent history
   - `git diff --stat HEAD~1`: last change summary

2. **Emit** an `## primed: statecrafting/.github` summary block (layer overview,
...
5. **Run the gate before every commit.** The governance floor, in this
   ...
   Two departures from the kit's commented defaults, in opposite directions.

   `--fail-on-unresolved --fail-on-warn` is **on**. The kit leaves it off
   because a corpus that ratifies before it builds legitimately carries
   unresolved units while work is under way (spec 050). This corpus has nothing
   to build: a unit it claims either exists on disk or was never written, so
   there is no such window and an unresolved claim is a defect on arrival.

   `index coverage` runs **without** `--fail-on-untraced`. The ownership
   ratchet walks the source files under a discovered package, and there is no
   manifest here to discover one from, so the flag exits 1 saying "nothing was
   verified" rather than 0 (spec 080: a gate that cannot ask says so). The read
   stays because its output is still the honest answer. `[coupling]
   require_ownership` is off to match, so every skill that conditions the flag
   on that key agrees with this list without being edited.
   ...
   There is no stack gate after it. This repository carries no code, so
   `make test build fmt clippy` are clean no-ops on a manifest probe and the
   governance floor is the whole gate. All five lines must exit 0. Commit the
   regenerated shards with the change that made them stale.

   `make gate` runs the read-only form of exactly this chain, and
   `.github/workflows/govern.yml` runs `make gate`. Keep the three identical:
   the skills tell their reader to run "the gate as `AGENTS.md` lists it", so a
   step CI enforces and this list omits is a step every session skips. This
   list is the definition; the other two follow it.
...
> There is no domain-specialist agent here and there should not be: a
> specialist loads a framework's reference docs, and this corpus has no
> framework. The domain knowledge that matters is the Statecraft family's own
> shape, and that lives in `profile/README.md`, which every session reads
> anyway.
...
Everything above this section is repository-invariant, and every `SKILL.md`
reads its project specifics from here.

**The binary.** `spec-spine` on `PATH`. There is nothing to build in-tree, so a
session that cannot find it runs `/setup` rather than looking for a
`target/release/` copy.

**The version pin.** `>=0.18.0`, a floor rather than an exact pin, set in
`spec-spine.toml [meta] required_version`. The CLI enforces it on every run, so
the manual `spec-spine --version` step in the init protocol is belt and braces
here rather than the only guard. 0.18.0 is the floor because the hooks in
`.claude/settings.json` call `spec-spine check` (spec 075) and `/verify` wraps
`spec-spine verify` (spec 049).

**The stack gate.** None. No Cargo workspace, no npm package, no test suite.
The governance floor under "Working the backlog" is the entire gate.

**Never touch by hand:**

- `.derived/**`: compiler output. Regenerate with `spec-spine compile` and
  `spec-spine index`, commit the result, never edit a shard to agree with a
  tree (`.claude/rules/derived-artifacts-are-compiler-output.md`).
- `.derived/**/build-meta.json`: gitignored wall-clock metadata. It is the one
  non-deterministic artifact and must never be committed.
- An approved spec, after the fact, to ratify what a change happened to do
  (`.claude/rules/adversarial-prompt-refusal.md`).

**The corpus and its territory.**

| Path | Owning spec |
| --- | --- |
| `spec-spine.toml`, `standards/`, `.claude/rules/`, `.gitignore`, `.gitattributes` | `000-bootstrap` |
| `README.md`, `profile/README.md`, `profile/artifacts/` | `001-the-profile-is-the-org-front-door` |
| `AGENTS.md`, `CLAUDE.md`, `.claude/settings.json`, `.claude/skills/`, `.claude/agents/`, `.mcp.json`, `Makefile`, `.github/workflows/govern.yml`, `.githooks/` | `002-the-harness-is-governed-like-the-work` |

What binds those paths is **C-001**: a change to any of them without an
authoring edit to its owning spec is refused at `couple` time. Give a new file
an ownership edge on the spec that introduces it, in the same change, and the
gate clears; touch a unit another spec owns and declare an `extends` edge on
your own spec naming that spec and unit.

`[coupling] require_ownership` (C-002) is off, and the reason is in
`spec-spine.toml`: it refuses an unclaimed **source** file, and this repository
has no source tree for it to walk. Ownership discipline here is held by C-001
and by review, not by the ratchet.

**What renders publicly.** `profile/README.md` is the org landing page at
github.com/statecrafting. An edit to it is visible to everyone the moment it
lands on the default branch, which is the reason it is claimed by a spec and
not treated as scratch prose.
