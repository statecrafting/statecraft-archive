...
| Your code | `addon/`, `packages/`, `vendor/encore/` | `npm run typecheck`; `npm run build` in the addon touched |
...
After each change:
- **Code**: run your project's build check (`npm run typecheck` for the TypeScript surface; `npm run build` inside the addon for a Rust change)
