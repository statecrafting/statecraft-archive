...
#   make typecheck build      the npm stack gate AGENTS.md lists after the floor
#
...
.PHONY: gate refresh verify test typecheck build fmt clippy help
...
test:
	...
	@if [ -f package.json ]; then npm test --if-present; else echo "no package.json, skipping"; fi

## The stack gate AGENTS.md lists after the governance floor. `npm run build`
## bakes the registry payload from the family's public repos and prerenders the
## site, so it is the real build and not a compile check.
typecheck:
	@if [ -f package.json ]; then npm run typecheck; else echo "no package.json, skipping"; fi

build:
	@test -f Cargo.toml && cargo build --workspace --locked || echo "no Cargo.toml, skipping"
	@if [ -f package.json ]; then npm run build; else echo "no package.json, skipping"; fi
...
help:
	...
	@echo "typecheck build   the stack gate (npm)"
	@echo "test fmt clippy   guarded on a manifest probe"
