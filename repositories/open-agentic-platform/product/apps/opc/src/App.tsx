...
function AppContent() {
  ...
  // Listen for quick-pane-submit events from the floating quick-pane window.
  // When the user submits text from the quick pane, forward it to the active
  // chat session by dispatching a synthetic window event.
  const handleQuickPaneSubmit = useCallback((text: string) => {
    window.dispatchEvent(new CustomEvent("opc:quick-pane-inject", { detail: { text } }));
  }, []);
  ...
  useEffect(() => {
    ...
    import("@tauri-apps/api/event").then(({ listen }) => {
      listen<{ text: string }>("quick-pane-submit", (event) => {
        handleQuickPaneSubmit(event.payload.text);
      }).then((fn) => { unlisten = fn; });
    });
    return () => { unlisten?.(); };
  }, [handleQuickPaneSubmit]);
  ...
  return (
    <div className="h-screen flex flex-col">
      ...
      {/* Spec 112 §6.3 — surface opc://project/open handoffs from statecraft. */}
      <ProjectOpenInbox
