...
export default defineConfig(async () => ({
  ...
  resolve: {
    ...
    // Allow files from packages/ui/src to resolve their deps from desktop's node_modules
    modules: [
  ...
  build: {
    ...
    rollupOptions: {
      // Multi-entry: main SPA + quick-pane floating window
      input: {
        main: path.resolve(fileURLToPath(new URL(".", import.meta.url)), "index.html"),
        "quick-pane": path.resolve(
